...
CREATE TABLE `license_log` (
  `license_logID` bigint NOT NULL AUTO_INCREMENT,
  ...
  `acceptanceNo` varchar(45) NOT NULL,
  `acceptanceStartDate` date NOT NULL,
  `acceptanceEndDate` date NOT NULL,
  `deletedBy` bigint DEFAULT NULL,
  `deletedAt` datetime DEFAULT NULL,
  PRIMARY KEY (`license_logID`),
  KEY `createdBy_licenseLog_idx` (`created_by`),
  KEY `updatedBy_licenseLog_idx` (`updated_by`),
  KEY `licenseID_licenseLog_idx` (`licenseID`),
  KEY `projectCharityID_licenseLog_idx` (`projectCharityID`),
  KEY `deletedBy_license_log` (`deletedBy`)
